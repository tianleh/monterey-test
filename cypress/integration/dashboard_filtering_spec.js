...
describe('dashboard filtering', () => {
  before(() => {
    ...
    testFixtureHandler.importJSONMapping('cypress/fixtures/dashboard/data/mappings.json.txt')

    testFixtureHandler.importJSONDoc('cypress/fixtures/dashboard/opensearch_dashboards/data.json.txt')
    testFixtureHandler.importJSONDoc('cypress/fixtures/dashboard/data/data.json.txt')
  })
  ...
  describe('Adding and removing filters from a dashboard', () => {
    before(() => {
      ...
      dashboardPage.addDashboardPanels('Filter Bytes Test', 'visualization', true)
    ...
    describe('adding a filter that excludes all data', () => {
      ...
      it('Nonpinned filter: data tables are filtered', () => {
        ...
        commonUI.checkElementDoesNotExist('[data-test-subj="paginated-table-body"] [data-cell-content]')
      })

      it('Nonpinned filter: goal and guages are filtered', () => {
        // Goal label should be 0, gauge label should be 0%
        commonUI.checkValuesExistInComponent('svg > g > g > text.chart-label', ['0', '0%'])
      })
      ...
      it('Nonpinned filter: tag cloud values are filtered', () => {
        commonUI.checkElementComponentDoesNotExist('[data-test-subj="tagCloudVisualization"]', 'svg > g > text')
      })

      it('Nonpinned filter: tsvb metric is filtered', () => {
        commonUI.checkValuesExistInComponent('[data-test-subj="tsvbMetricValue"]', ['0 custom template'])
      })

      it('Nonpinned filter: tsvb top n is filtered', () => {
        commonUI.checkElementContainsValue('[data-test-subj="tsvbTopNValue"]', 2, '0')
      })
    ...
    describe('disabling a filter unfilters the data on', () => {
      ...
      it('Filter disabled: data tables', () => {
        ...
        commonUI.checkElementExists('[data-test-subj="paginated-table-body"] [data-cell-content]', 10)
      })
      ...
      it('Filter disabled: goal and guages', () => {
        ...
        // data loading issue.
        commonUI.checkValuesExistInComponent('svg > g > g > text.chart-label', ['7,544', '39.958%'])
      })
      ...
      it('Filter disabled: tag cloud', () => {
        commonUI.checkValuesExistInComponent('[data-test-subj="tagCloudVisualization"]', ['9,972', '4,886', '1,944', '9,025'])
      })

      it('Filter disabled: tsvb metric', () => {
        commonUI.checkValuesExistInComponent('[data-test-subj="tsvbMetricValue"]', ['50,465 custom template'])
      })

      it('Filter disabled: tsvb top n', () => {
        commonUI.checkElementContainsValue('[data-test-subj="tsvbTopNValue"]', 2, '6,308.125')
      })

      it('Filter disabled: tsvb markdown', () => {
        commonUI.checkValuesExistInComponent('[data-test-subj="tsvbMarkdown"]', ['7,209.286'])
      })

      it('Filter disabled: saved search is filtered', () => {
        commonUI.checkElementExists('[data-test-subj="docTableExpandToggleColumn"]', 1)
      })
  ...
  describe('nested filtering', () => {
    before(() => {
      ...
      dashboardPage.addDashboardPanels('Rendering Test: animal sounds pie', 'visualization', false)
    })
    ...
    it('Pie chart linked to saved search filters data', () => {
      dashboardPage.addDashboardPanels('Filter Test: animals: linked to search with filter', 'visualization', false)
